...
bool mygame_state_init() {
  ...
  // initialize sdl ttf
  if (!TTF_Init()) {
    fprintf(stderr, "Failed to initialize SDL TTF: %s\n", SDL_GetError());
    return false;
  }

  gbl_state = malloc(sizeof(struct mygame_state));
  if (gbl_state == NULL) {
    fprintf(stderr, "Something Wrong happened and State is invalid: %s\n",
            SDL_GetError());
    return false;
  }
  ...
  if (gbl_state->fonts == NULL) {
    fprintf(stderr, "Failed to load mygame_fonts: %s\n", SDL_GetError());
    return false;
  }
  ...
  if (gbl_state->images == NULL) {
    fprintf(stderr, "Failed to load mygame_images: %s\n", SDL_GetError());
    return false;
  }
