...
bool mygame_state_init(struct mygame_assets_manager* assets_manager) {
  // check if assets_manager is implemented
  if (assets_manager == NULL || assets_manager->read_text_file == NULL ||
      assets_manager->read_bin_file == NULL ||
      assets_manager->file_exists == NULL ||
      assets_manager->list_files == NULL ||
      assets_manager->free_file_list == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Please implement all assets_manager functions before "
                 "initialize state.\n");
    return false;
  }
  ...
  if (gbl_state == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Something Wrong happened and State is invalid: %s\n",
                 SDL_GetError());
    return false;
  }

  gbl_state->assets_manager = assets_manager;
  ...
  if (gbl_state->images == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Failed to load mygame_images: %s\n", SDL_GetError());
    return false;
  }
